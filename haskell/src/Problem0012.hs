module Problem0012 (solution) where
...
solution =
  E.ProblemSolution
    { E.psNr = 12,
      E.psTitle = "Highly Divisible Triangular Number",
      E.psSolve = \_ -> show out,
      E.psSolution = "",
      E.psReadInput = pure ""
    }
...
allFactors n = (E.unique . concatMap pairDivs) allSmallDivs
  where
    allSmallDivs = filter (\x -> n `mod` x == 0) (n : [1 .. ceiling (sqrt (fromIntegral n))])
...
maybeSolution = findFirst (\tns -> length tns > moreThanFactors) (map allFactors triangleNumbersList)

out = case maybeSolution of
  Nothing -> "No solution"
