...
-- | Solution without memoization - for now
maxSum :: (Int, Int) -> E.Map2d Int -> Int
maxSum (y, x) valueMap = actValue
  where
    actValue = case Map.lookup (y, x) valueMap of
      Nothing -> 0
      Just val -> val + greaterVal
    leftVal = maxSum (y + 1, x) valueMap
    rightVal = maxSum (y + 1, x + 1) valueMap
